...
import {LoadState, QueueState, RedisCrawlState} from "./util/state.js";
...
import {checkDiskUtilization, getDirSize, getFileSize, initStorage, interpolateFilename} from "./util/storage.js";
import {RedisPubSubTransport, ScreenCaster, WSTransport} from "./util/screencaster.js";
import {Screenshots} from "./util/screenshots.js";
import {parseArgs} from "./util/argParser.js";
import {initRedis} from "./util/redis.js";
import {errJSON, logger} from "./util/logger.js";
...
import {ADD_LINK_FUNC, BEHAVIOR_LOG_FUNC, DEFAULT_SELECTORS, HTML_TYPES} from "./util/constants.js";
...
export class Crawler {
  constructor() {
    ...
    this.maxPageTime = this.params.pageLoadTimeout + this.params.behaviorTimeout + FETCH_TIMEOUT_SECS * 2 + PAGE_OP_TIMEOUT_SECS * 2 + this.params.pageExtraDelay;
    ...
    this.gotoOpts = {
      waitUntil: this.params.waitUntil, timeout: this.params.pageLoadTimeout * 1000
    };
  ...
  async bootstrap() {
    ...
    if (!this.params.headless && !process.env.NO_XVFB) {
      child_process.spawn("Xvfb", [process.env.DISPLAY, "-listen", "tcp", "-screen", "0", process.env.GEOMETRY, "-ac", "+extension", "RANDR"]);
    }
  }
  ...
  async setupPage({page, cdp, workerid, callbacks}) {
    ...
    if ((this.adBlockRules && this.params.blockAds) || this.blockRules || this.originOverride) {
...
`;
  ...
  async crawlPage(opts) {
    ...
    if (this.params.behaviorOpts) {
      if (!data.isHTMLPage) {
        ...
        const res = await timedRun(this.runBehaviors(page, cdp, data.filteredFrames, logDetails), this.params.behaviorTimeout, "Behaviors timed out", logDetails, "behavior");
  ...
  async runBehaviors(page, cdp, frames, logDetails) {
    try {
      ...
      logger.info("Running behaviors", {
        frames: frames.length, frameUrls: frames.map(frame => frame.url()), ...logDetails
      }, "behavior");

      const results = await Promise.allSettled(frames.map(frame => this.browser.evaluateWithCLI(page, frame, cdp, `
  ...
  async createWARCInfo(filename) {
    ...
    const info = {
      "software": this.infoString, "format": "WARC File Format 1.0"
    };
  ...
  async crawl() {
    ...
    console.log("Selected User Agent:", process.env.SELECTED_UA || this.emulateDevice.userAgent);
    await this.browser.launch({
      ...
      chromeOptions: {
        ...
        userAgent: process.env.SELECTED_UA || this.emulateDevice.userAgent,
  ...
  async generateWACZ() {
    ...
    const createArgs = ["create", "--split-seeds", "-o", waczPath, "--pages", this.pagesFile, "--log-directory", this.logDir];
  ...
  async loadPage(page, data, selectorOptsList = DEFAULT_SELECTORS) {
    ...
    let isHTMLResult = await timedRun(this.isHTML(url), FETCH_TIMEOUT_SECS, "HEAD request to determine if URL is HTML page timed out", logDetails, "fetch", true);

    let isHTMLPage = isHTMLResult.isHTML;
    data.statusCode = isHTMLResult.statusCode;  // Storing the status code in the data object


    if (!isHTMLPage) {
      try {
        const captureResult = await timedRun(this.directFetchCapture(url), FETCH_TIMEOUT_SECS, "Direct fetch capture attempt timed out", logDetails, "fetch", true);
        if (captureResult) {
  ...
  async extractLinks(page, data, selectors = DEFAULT_SELECTORS, logDetails) {
    ...
    try {
      for (const {selector = "a[href]", extract = "href", isAttribute = false} of selectors) {
        const promiseResults = await Promise.allSettled(frames.map(frame => timedRun(frame.evaluate(loadLinks, {
          selector, extract, isAttribute, addLinkFunc: ADD_LINK_FUNC
        }), PAGE_OP_TIMEOUT_SECS, "Link extraction timed out", logDetails,)));

        for (let i = 0; i < promiseResults.length; i++) {
          const {status, reason} = promiseResults[i];
          if (status === "rejected") {
            logger.warn("Link Extraction failed in frame", {
              reason, frameUrl: frames[i].url, ...logDetails
            });
          }
        }
      }
  ...
  async checkCF(page, logDetails) {
    try {
      ...
      while (await timedRun(page.$("div.cf-browser-verification.cf-im-under-attack"), PAGE_OP_TIMEOUT_SECS, "Cloudflare check timed out", logDetails, "general", true)) {
  ...
  async parseSitemap(url, seedId, sitemapFromDate) {
    ...
    const sitemapper = new Sitemapper({
      url, timeout: 15000, requestHeaders: this.headers, lastmod: lastmodFromTimestamp
    });
...
function shouldIgnoreAbort(req) {
  try {
    ...
    if (headers["content-disposition"] || (headers["content-type"] && !headers["content-type"].startsWith("text/"))) {
      return true;
    }
  } catch (e) {
    return false;
